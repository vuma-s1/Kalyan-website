...
const PersonalInsights = () => {
  ...
  return (
    <section id="personal-insights" className="py-8 lg:py-24 relative overflow-hidden" style={{ backgroundColor: '#ffe5d4' }}>
      ...
      <div className="container mx-auto px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 max-w-7xl mx-auto">
          {quotes.map((quote, index) => (
            <div 
              key={index}
                          className="rounded-xl shadow-lg border border-yellow-200 p-4 text-center hover:shadow-xl transition-all duration-500 hover:-translate-y-1 animate-fade-in-up relative overflow-hidden"
            style={{ backgroundColor: '#ce6f69', animationDelay: `${index * 0.15}s` }}
            >
              {/* Quote Icon */}
                              <div className="w-10 h-10 rounded-lg flex items-center justify-center mx-auto mb-3 shadow-md" style={{ backgroundColor: 'rgba(155, 53, 52, 0.85)' }}>
              ...
              {/* Quote Text */}
                              <blockquote className="text-sm leading-relaxed italic mb-3" style={{ fontFamily: 'FocusGrotesk, sans-serif', fontWeight: 'normal', color: '#ffe5d4' }}>
                "{quote}"
              </blockquote>
              
              {/* Decorative Element */}
                              <div className="absolute bottom-2 right-2 w-4 h-4 opacity-20 rounded-full" style={{ backgroundColor: 'rgba(155, 53, 52, 0.85)' }}></div>
            </div>
          ))}
        </div>
      </div>
